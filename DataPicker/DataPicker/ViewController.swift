...
class ViewController: UIViewController {
    ...
    @objc func updateTime() { // 타이머 동작 함수
        ...
        let date = NSDate() // NSDAte 함수를 사용하여 현재시간을 가져옴
        ...
        if(lblPickerTime.text == lblCurrentTime.text) { // lblPickerTime.text와 lblCurrentTime.text가 같은지 확인하는 분기
            view.backgroundColor = UIColor.red // 조건이 true일 view의 배경화면을 빨간색으로 변경
        } else {
